		...
		while(stopMainValue != true) {
			...
			for(int i = 0; i < allPeople.size(); i++) {
				if(allPeople.get(i).getString("person_type").equals("Citizen")) {
					...
					if(allPeople.get(i).getString("state").equals("active")) {
						
						int maxCrimeValue = (-1) * dimension * dimension;
						
						for(int j = 0; j < allPossibleLocations.size(); j++) {
							
							boolean isCloseToARebel = citizen.isCloseToAnyRebel(vision, city, 
									allPossibleLocations.get(j).getInt("x_coordinate"),
									allPossibleLocations.get(j).getInt("y_coordinate"), dimension);
							
							if(isCloseToARebel == true) {
								int [] nearbyPersonTypeCounts = citizen.countActiveCitizens(vision, city, 
										allPossibleLocations.get(j).getInt("x_coordinate"),
										allPossibleLocations.get(j).getInt("y_coordinate"), dimension);
								
								if((nearbyPersonTypeCounts[0] - nearbyPersonTypeCounts[1]) > maxCrimeValue) {
									maxCrimeValue = nearbyPersonTypeCounts[0] - nearbyPersonTypeCounts[1];
									coordinates = allPossibleLocations.get(j);
								}
							}
						}
					}
